...
class AOTInductorModelContainer {
  ...
  AOTInductorModelContainer(
    ...
    for (size_t i = 0; i < num_constants; i++) {
      ...
      uint8_t* internal_ptr = constant_ptr(
          is_cpu, constants_internal_offset[i], bytes_read, data_size);
 ...
 private:
  ...
  uint8_t* constant_ptr(bool is_cpu, size_t constant_offset,
      size_t bytes_read, size_t data_size) {
    ...
    if (is_cpu) {
      ...
      internal_ptr = constants_ptr + constant_offset;
  ...
  void make_cuda_constant_blob(AOTInductorModel *model,
